 ...
 */
function ug_remove_cpt_slug( $post_link, $post, $leavename ) {
 
    if ( ! in_array( $post->post_type, array( 'sponso' ) ) || 'publish' != $post->post_status )
        return $post_link;
...
add_filter( 'post_type_link', 'ug_remove_cpt_slug', 10, 3 );
 ...
 */
function ug_parse_request_tricksy( $query ) {
    ...
    if ( ! empty( $query->query['name'] ) )
        $query->set( 'post_type', array( 'post', 'sponso', 'page' ) );
}
add_action( 'pre_get_posts', 'ug_parse_request_tricksy' );
